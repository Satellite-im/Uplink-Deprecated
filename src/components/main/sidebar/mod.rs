...
pub fn Sidebar(cx: Scope<Props>) -> Element {
    ...
    let mp = cx.props.account.clone();
    let ident = warp::async_block_in_place_uncheck(mp.get_own_identity()).expect("Unexpected error <temp>");
    ...
    let filtered_chats = chats.clone().into_iter().filter(|conv| {
            ...
            .is_none()
        {
            let username = conv
                ...
                .filter_map(|did| warp::async_block_in_place_uncheck(mp.get_identity(did.clone().into())).ok())
                .flatten()
                .map(|i| i.username())
                .last()
                .unwrap_or_default();
